...
//Function that takes care of the creation of the  page DeleteAnExam.js
export default function DeleteAnExam() {
    ...
    //in a json format and henceforth with the database as seen in the file app.js to remove an exam
    //The response sent by the express server is then jsonified and dealt with appropriately or throw an error if
    //an error is thrown
    function removeExam() {
        fetch('http://192.168.0.147:30000/removeExam', {
            method: 'POST',
            headers: { 
        ...
        .then((responseData) => { if(responseData.message == 'true') alert("Exam removed!"); else alert('Unable to remove exam!'); })
        .catch((err) => { alert(err); });
    }
    ...
    return(
        <ScrollView style={styles.wholePage}>
            ...
            <View style={styles.heading}>
                <Text style={styles.headingText}>Remove An Exam</Text>
            </View>
            ...
            {/*View/felxbox for the form to be filled to Delete an exam*/}
            <View style={styles.inputs}>
                ...
                <TouchableOpacity onPress={removeExam} style={styles.finishExamButton}>
                    <Text style={styles.finishExamButtonText}>Delete Exam</Text>
                </TouchableOpacity>
            </View>
        </ScrollView>
    );
